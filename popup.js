...
function render(friends) {
  ...
  for (var i = 0; i < friends.length; i++) {
    ...
    var record =
      ...
      ")</td>" +
      "<td>" +
      (f.published ? f.area : "(非公開)") +
      "</td>" +
      "<td class='playerMemo' title='クリックでピラミッド状況' id='memo" +
      f.webPcNo +
      "'>" +
      (f.published ? f.memo : "(非公開)") +
      "</td></tr>";
    html = html + record;
  ...
  var queryPyramid = function () {
    ...
    xhr.onload = function () {
      ...
      var imgs = xml.querySelectorAll(
        "#statusArea > div.pyramid > ul > li > img"
      );
      console.log(imgs);
      if (imgs.length === 0) {
...
function getFriendsByPage(webPcNo, pageNo, results, callback) {
  ...
  xhr.onload = function () {
    ...
    var xml = xhr.responseXML;
    var friends = xml.querySelectorAll(
      "#contentArea > div > div.bdBox1.myFriendList > div.article"
    );
    ...
    for (var i = 0; i < friends.length; i++) {
      ...
      var pcNo = reWebPcNo.exec(
        el.getElementsByTagName("dd")[0].getElementsByTagName("a")[0].href
      )[1];
    ...
    var nextLink = xml.querySelector(
      "#contentArea > div > div.bdBox1.myFriendList > div.pageNavi > ul > li.next > a"
    );
  ...
  xhr.open(
    "GET",
    "https://hiroba.dqx.jp/sc/character/" +
      webPcNo +
      "/friendlist/page/" +
      pageNo
  );
